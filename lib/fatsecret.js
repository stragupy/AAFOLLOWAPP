...
async function searchFoods(query, limit = 12) {
  ...
  let data;
  try {
    data = await fatSecretGet('/foods/search/v5', params);
  } catch (error) {
    if (!(error instanceof FatSecretError) || !['FATSECRET_SCOPE_REQUIRED'].includes(error.code)) throw error;
    data = await fatSecretGet('/foods/search/v1', params);
  }
